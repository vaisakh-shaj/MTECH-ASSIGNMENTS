...
SAMPLE      read_struct_examples(char *file, STRUCT_LEARN_PARM *sparm)
{
  ...
  for(i=0;i<n;i++){     /* find highest class label */
    yval=get_target_class(target[i]);
    if(max_class < yval) 
     max_class=yval;
  }
  num_classes=pow(2,max_class)+0.1;
  ...
  for(i=0;i<n;i++) {          /* copy docs over into new datastructure */
    ...
    examples[i].y.max_label=max_class;
  }
  free(target);
  free(docs);
  sample.n=n;
			      ...
			      KERNEL_PARM *kparm)
{
  ...
  for(i=0;i<sample.n;i++){     /* find highest class label */
    yval=get_target_class(sample.examples[i].y.class);
    if(max_class < yval) 
     max_class=yval;
  }
  sparm->num_classes=pow(2,max_class)+0.1;;
  sm->max_label=max_class+0.1;
  ...
  sm->sizePsi=sparm->num_features*sm->max_label;
  if(struct_verbosity>=0)
				    ...
				    STRUCT_LEARN_PARM *sparm)
{
  ...
  double score,bestscore=-1,score_sum=0.0;
  WORD *words;
  ...
  y.class=bestclass;
  y.max_label=sm->max_label;
  score_sum=score_sum/sm->max_label;
  return(y);
}
						     ...
						     STRUCT_LEARN_PARM *sparm)
{
  ...
  /*ybar.scores=NULL;*/
  ybar.scores=(double *)my_malloc(sizeof(double)*(sparm->num_classes+1));
  ...
  for(class=1;class<=sparm->num_classes;class++) {
    ...
    free_svector(doc.fvec);
    score=loss(y,ybar,sparm)*(1.0-score_y+score_ybar); /*slack variable*/
    ybar.scores[class]=score_ybar;
    score_sum+=score_ybar;
    if((bestscore<score)  || (first)) {
      bestscore=score;
      bestclass=class;
      first=0;
  ...
  score_sum=score_sum/sparm->num_classes;
		 ...
		 STRUCT_LEARN_PARM *sparm)
{
  ...
  int num=y.class,remainderr,var=0,count=0;
  SVECTOR *fvec,*fvec_temp;
  ...
  while(num>0){
    ...
    if(remainderr==1){
      if(count==0){
        fvec=shift_s(x.doc->fvec,(var-1)*sparm->num_features);
        count++;
      }
      else{
        count++;
        fvec_temp=shift_s(x.doc->fvec,(var-1)*sparm->num_features);
        ai=fvec->words;
        bj=fvec_temp->words;
        sum=(WORD *)my_malloc(sizeof(WORD)*(count*sparm->num_features+1));
        sumi=sum;
        while (ai->wnum) {
          (*sumi)=(*ai);
          sumi++;
        ...
        sumi->wnum=0;
        fvec=create_svector_shallow(sum,NULL,1.0);
        //free(sum);    
        //fvec=add_ss(fvec,fvec_temp);  
      }
    }
    num=num/2;
  }
			       ...
			       STRUCT_LEARN_PARM *sparm)
{
  ...
  { perror (file); exit (1); }
  fprintf(modelfl,"SVM-multilabel Version %s\n",INST_VERSION);
...
STRUCTMODEL read_struct_model(char *file, STRUCT_LEARN_PARM *sparm)
{
  ...
  fscanf(modelfl,"SVM-multilabel Version %s\n",version_buffer);
...
void        write_label(FILE *fp, LABEL y, int max_class)
{
  /* Writes label y to file handle fp. */
  /*
  int i,num,var=1,remainderr;
  //fprintf(fp,"%d",y.class);
  fprintf(fp,"[ ");
  //for(i=1;i<=y.num_classes;i++)
  //  fprintf(fp,"%d ",y.classes[i]);
  //fprintf(fp,"] %d ::: ",y.class);
  num=y.class;
  while (num>0){
    remainderr=num%2;
    fprintf(fp,"%d ",remainderr);
    var++;
    num=num/2;
  }
  while(var<=max_class){
...
int       get_target_class(int label)
{
  /*  To get the maximum class label in a binary representation of
      the target value of a multi-label problem */
  int num,remainderr,no_of_1s=0;
...
{
  /* check whether a no is a power of two */
  int i;
  if(num==1)
    return 1;
  for(i=1;i<num;i++){
